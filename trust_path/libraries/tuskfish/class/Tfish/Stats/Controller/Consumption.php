...
/**
 * \Tfish\Stats\Controller\Consumption class file.
 *
...
/**
 * Controller for the consumption page (/consumption/).
 *
 * @copyright   Simon Wilkinson 2026+ (https://tuskfish.biz)
 * @license     https://www.gnu.org/licenses/old-licenses/gpl-2.0.en.html GNU General Public License (GPL) V2
 * @author      Simon Wilkinson <simon@isengard.biz>
...
class Consumption
{
    ...
    /**
     * Render the consumption page.
     *
     * The country/species/year parameters do not filter this map; they are carried only so the
     * persisted dashboard selection survives a visit here. A request carrying any of them is rendered
     * fresh (not cached) so the embedded carry-state seed is never served stale, matching the other
     * parameterised Stats pages.
     *
    ...
    public function display(): array
    {
        ...
        $cacheParams = ($country !== '' || $species !== '' || $year !== '') ? [] : ['page' => 'consumption'];
        ...
        $this->viewModel->displayConsumption($country, $species, $year);

        return $cacheParams;
    }

    /**
     * Return the consumption payload as JSON (AJAX endpoint; reserved for the dynamic version).
     *
    ...
    public function consumptiondata(): array
    {
        ...
        try {
            ...
            echo \json_encode($this->model->consumptionData(), JSON_THROW_ON_ERROR);
        } catch (\Throwable $e) {
            $this->logger->logError((int) $e->getCode(), $e->getMessage(), $e->getFile(), (int) $e->getLine());
            \http_response_code(500);
            echo \json_encode(['error' => 'Failed to load consumption data']);
        }
