...
/**
 * \Tfish\Stats\Controller\Trade class file.
 *
...
/**
 * Controller for the trade page (/trade/).
 *
 * @copyright   Simon Wilkinson 2026+ (https://tuskfish.biz)
 * @license     https://www.gnu.org/licenses/old-licenses/gpl-2.0.en.html GNU General Public License (GPL) V2
 * @author      Simon Wilkinson <simon@isengard.biz>
...
class Trade
{
    ...
    public function display(): array
    {
        ...
        $cacheParams = ($country !== '') ? [] : ['page' => 'trade'];
        ...
        $this->viewModel->displayTrade($country);

        return $cacheParams;
    }

    /**
     * Return the trade payload as JSON (AJAX endpoint).
     *
    ...
    public function tradedata(): array
    {
        ...
        try {
            ...
            echo \json_encode($this->model->tradeData(), JSON_THROW_ON_ERROR);
        } catch (\Throwable $e) {
            $this->logger->logError((int) $e->getCode(), $e->getMessage(), $e->getFile(), (int) $e->getLine());
            \http_response_code(500);
            echo \json_encode(['error' => 'Failed to load trade data']);
        }
    ...
    /**
     * Return the list of countries that report trade as JSON (AJAX endpoint).
     *
     * @return  array Unused; the method emits JSON and exits.
     */
