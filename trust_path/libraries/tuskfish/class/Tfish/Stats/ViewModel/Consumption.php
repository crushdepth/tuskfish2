...
/**
 * \Tfish\Stats\ViewModel\Consumption class file.
 *
...
/**
 * ViewModel for the consumption page (/consumption/).
 *
 * Renders a single global choropleth of per-capita apparent aquatic food consumption. The country,
 * species and year dimensions have no effect on this map; they are accepted only as pass-through
 * state so that a selection made on another tab survives a visit here and is carried back. They are
 * therefore deliberately NOT appended to the page title (buildMetadata is called with no filters):
 * the canonical URL stays /consumption/ and the title never implies country-specific data.
 *
 * @copyright   Simon Wilkinson 2026+ (https://tuskfish.biz)
 * @license     https://www.gnu.org/licenses/old-licenses/gpl-2.0.en.html GNU General Public License (GPL) V2
 * @author      Simon Wilkinson <simon@isengard.biz>
 ...
 */
class Consumption implements \Tfish\Interface\Viewable
{
    ...
    public function __construct(
        ...
        $this->pageTitle = TFISH_STATS_CONSUMPTION_TITLE;
        $this->description = TFISH_STATS_CONSUMPTION_DESCRIPTION;
    }

    /**
     * Render the consumption template.
     *
     ...
     */
    public function displayConsumption(string $country = '', string $species = '', string $year = ''): void
    {
        $this->template = "stats-consumption";
        $this->carriedState = [
            'country' => $this->trimString($country),
            'species' => $this->trimString($species),
            'year' => $this->trimString($year),
        ];
        $this->model->loadConsumptionData($country);
    ...
    /**
     * Consumption payload as JSON for the initial page render.
     *
     * @return  string JSON array of ['id' => int, 'name' => string, 'kg' => float].
     */
    public function consumptionDataJson(): string
    {
        return \json_encode(
            $this->model->consumptionData(),
