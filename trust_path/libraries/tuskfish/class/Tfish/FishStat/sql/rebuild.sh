#!/usr/bin/env bash
#
# rebuild.sh — rebuild ALL derived FishStat summary tables (and their indexes) in one step.
#
# Run this after replacing or updating the FishStat database (e.g. an annual FAO refresh).
...
[ "$#" -eq 1 ] || die "usage: $(basename "$0") /path/to/fishstat.db"
DB="$1"
...
printf 'Rebuilding FishStat summary tables in: %s\n\n' "$DB"
...
# The covering index that backs the /producers/ species filter is built inside
