...
/**
 * \Tfish\FishStat\Model\Producers class file.
 *
...
/**
 * Model for the aquaculture producers page (/producers/).
 *
 ...
 * @uses        trait \Tfish\FishStat\Traits\FishStatDatabase  Connection and species/country lookup helpers.
 */
class Producers
{
    ...
    {
        $this->producersData = $this->buildLanding($this->latestYear());
    }
     ...
     */
    public function loadProducersData(string $speciesCode, int $year): void
    {
        if (!$this->fishStatDb) {
            $this->producersData = $this->emptyLanding($year);
            return;
        }
