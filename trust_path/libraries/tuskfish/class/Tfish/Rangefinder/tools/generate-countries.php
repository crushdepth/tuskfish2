...
/**
 * Generator for Tfish\Rangefinder\Traits\RangefinderCountries.
 *
 * Reads the Debian iso-codes package and emits the PHP trait. Same rule as the JS table it
 * replaces: each entry's short common name where it defines one, its formal name otherwise.
 *
 *   php gen_countries.php > .../Traits/RangefinderCountries.php
 */

declare(strict_types=1);

$source = '/usr/share/iso-codes/json/iso_3166-1.json';
$json = \json_decode(\file_get_contents($source), true, 512, JSON_THROW_ON_ERROR);

$names = [];

foreach ($json['3166-1'] as $entry) {
    $code = $entry['alpha_2'] ?? '';

    if ($code === '') continue;

    $names[$code] = $entry['common_name'] ?? $entry['name'];
}
